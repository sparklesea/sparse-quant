...
def quant_muxi(weight, input_features, output_features, n_bit, group_size):

    reformat_weight = weight.reshape(-1, 4, 2).permute(0, 2, 1).reshape(output_features, input_features).clone()
    qweight, zeros_scales = generate_quant_muxi(reformat_weight, input_features, group_size, n_bit)
...
def write_2bit_tensor_muxi(w_q):
    w_q_org_shape = w_q.shape
...
def generate_quant_muxi(weight_fp16, input_features, group_size, q_bit):
    ...
    qweight_temp, scales_temp, zeros_temp = quantize_tensor_muxi(weight_fp16[:, :end], q_bit, group_size)

    if q_bit==4:
        intweight.append(write_4bit_tensor_muxi(qweight_temp))
    elif q_bit==2:
        intweight.append(write_2bit_tensor_muxi(qweight_temp))
