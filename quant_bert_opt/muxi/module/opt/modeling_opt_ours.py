...
class OPTAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = WALinear(embed_dim, embed_dim, bias=bias, dtype=torch.uint8)
        self.v_proj = WALinear(embed_dim, embed_dim, bias=bias, dtype=torch.uint8)
        if not is_last:
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig, is_last: bool = False):
        ...
        self.fc1 = WALinear(self.embed_dim, config.ffn_dim, bias=config.enable_bias, dtype=torch.uint8)
        self.fc2 = WALinear(config.ffn_dim, self.embed_dim, bias=config.enable_bias, dtype=torch.uint8)
...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def __init__(self, config: OPTConfig):
        ...
        # self.layers = nn.ModuleList([OPTDecoderLayer(config, False)] * (config.num_hidden_layers - 1) + [OPTDecoderLayer(config, True)])
