...
__global__ __forceinline__ void w4a16_gemm_wmma_kernel_origin(uint32_t* __restrict__ qweight,
    ...
    mxmaca::wmma::fragment<mxmaca::wmma::matrix_a, 16, 16, 16, __half, mxmaca::wmma::row_major> frag_a;
    mxmaca::wmma::fragment<mxmaca::wmma::matrix_b, 16, 16, 16, __half, mxmaca::wmma::col_major> frag_b;
    mxmaca::wmma::fragment<mxmaca::wmma::accumulator, 16, 16, 16, __half> frag_c;

    mxmaca::wmma::fill_fragment(frag_c, __float2half(0.0f));
    ...
    for (int bk = 0; bk < K / BK; bk++) {
        ...
        mxmaca::wmma::load_matrix_sync(frag_a, &s_a[wid * 16], LDK);
        mxmaca::wmma::load_matrix_sync(frag_b, &s_b[wid * 16], LDK);
        mxmaca::wmma::mma_sync(frag_c, frag_a, frag_b, frag_c);

        mxmaca::wmma::load_matrix_sync(frag_a, &s_a[wid * 16 + 64], LDK);
        mxmaca::wmma::load_matrix_sync(frag_b, &s_b[wid * 16 + 64], LDK);
        mxmaca::wmma::mma_sync(frag_c, frag_a, frag_b, frag_c);

        mxmaca::wmma::load_matrix_sync(frag_a, &s_a[wid * 16 + 128], LDK);
        mxmaca::wmma::load_matrix_sync(frag_b, &s_b[wid * 16 + 128], LDK);
        mxmaca::wmma::mma_sync(frag_c, frag_a, frag_b, frag_c);

        mxmaca::wmma::load_matrix_sync(frag_a, &s_a[wid * 16 + 192], LDK);
        mxmaca::wmma::load_matrix_sync(frag_b, &s_b[wid * 16 + 192], LDK);
        mxmaca::wmma::mma_sync(frag_c, frag_a, frag_b, frag_c);
    ...
    mxmaca::wmma::store_matrix_sync(&smem[wid * BM * LDN], frag_c, LDN, mxmaca::wmma::layout_t::mem_row_major);
