...
__global__ void WMMAF16TensorCore(float *A, float *B, float *C,
	     const int M, const int N, const int K, const int M_TOTAL, const int N_TOTAL, const int K_TOTAL)
{
	...
	nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 16, 16, 16, float, nvcuda::wmma::row_major> a_frag;
	nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, 16, 16, 16, float, nvcuda::wmma::row_major> b_frag;
...
__global__ void WMMAF16TensorCore(float *A, float *B, float *C)
{
    ...
    // Load the inputs
    nvcuda::wmma::load_matrix_sync(a_frag, A + wid * 256, 16);
    nvcuda::wmma::load_matrix_sync(b_frag, B + wid * 256, 16);

    __syncthreads();
    nvcuda::wmma::mma_sync(c_frag, a_frag, b_frag, c_frag);

    nvcuda::wmma::store_matrix_sync(smem + wid*256, c_frag, 16, nvcuda::wmma::mem_row_major);

    __syncthreads();

    C[tid] = smem[tid] + smem[tid+256] + smem[tid+256*2] + smem[tid+256*3];
}
...
__global__ void WMMAF16TensorCore_32(half *A, half *B, float *C)
{
	nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 16, 16, 32, __half, nvcuda::wmma::row_major> a_frag;
	nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, 16, 16, 32, __half, nvcuda::wmma::col_major> b_frag;
	// nvcuda::wmma::fragment<nvcuda::wmma::accumulator, M, N, K, float> ab_frag;
    ...
    // Load the inputs
    nvcuda::wmma::load_matrix_sync(a_frag, A, 32);
...
// //MMBI
// __global__ void WMMAF16TensorCore_MMBI(half *A, half *B, float *C)
// {
// 	nvcuda::wmma::FragmentARowB16<MMBI> a_frag;
// 	nvcuda::wmma::FragmentAColB16<MMBI> b_frag;
// 	nvcuda::wmma::fragmentIx<MMBI, nvcuda::wmma::accumulator, float> c_frag

//     // nvcuda::wmma::fill_fragment(c_frag, 0.0f);

//     // Load the inputs
//     nvcuda::wmma::load_matrix_sync_tcu(a_frag, A, 16, 0, 0);
//     nvcuda::wmma::load_matrix_sync_tcu(b_frag, B, 16, 0, 0);

//     nvcuda::wmma::mma_sync_tcu(c_frag, a_frag, b_frag, c_frag);
...
__global__ __forceinline__ void w4a16_gemm_wmma_kernel_32(uint32_t* __restrict__ qweight,
    ...
    nvcuda::wmma::store_matrix_sync(&smem_fp32[wid * BM * LDN], frag_c, LDN, nvcuda::wmma::mem_row_major);
...
template <int BM, int BN, int BK, int LDK, int LDN>
__global__ __forceinline__ void w4a16_gemm_wmma_kernel_float_origin(uint32_t* __restrict__ qweight,
                                                       half2* zeros_scales,
                                                       const half* __restrict__ input,
                                                       half* __restrict__ output,
                                                       const int M, const int N, const int K,
...
__global__ __forceinline__ void w4a16_gemm_wmma_kernel_float(uint32_t* __restrict__ qweight,
    ...
    // __shared__ float smem[(BM + BN) * LDK];
	__shared__ float smem[(BM + BN) * 256];
    float* s_a = smem;
    float* s_b = smem + BM * 256;

    nvcuda::wmma::fragment<nvcuda::wmma::matrix_a, 16, 16, 16, float, nvcuda::wmma::row_major> frag_a;
    nvcuda::wmma::fragment<nvcuda::wmma::matrix_b, 16, 16, 16, float, nvcuda::wmma::row_major> frag_b;
    nvcuda::wmma::fragment<nvcuda::wmma::accumulator, 16, 16, 16, float> frag_c;

    nvcuda::wmma::fill_fragment(frag_c, 0.0f);

	//for input
    int load_a_smem_m = tid % 16;
    int load_a_smem_k = (tid / 16) * 16;
    // 8 x 32, 2 times
	//for weight
    int load_b_smem_n = tid % 16;
    int load_b_smem_k = (tid / 16) * 16;

	float* load_a_smem_addrs = s_a + tid * 16;
	float* load_b_smem_addrs = s_b + load_b_smem_n + load_b_smem_k * 16;
    ...
    for (int bk = 0; bk < K / BK; bk++) {
        if (load_a_gmem_m < M) {
              *(float4*)(temp_load_half) = *(float4*)(&input[load_a_gmem_addr]);
              *((float4*)(temp_load_half+8)) = *((float4*)(&input[load_a_gmem_addr+8]));
              for(int i=0;i<8;++i){
                     temp_load_float[i] = __half2float(temp_load_half[i]);
                     temp_load_float[i + 8] = __half2float(temp_load_half[i + 8]);
              }
              *(float4*)(load_a_smem_addrs) = *(float4*)(&temp_load_float[0]);
              *((float4*)(load_a_smem_addrs + 4)) = *(float4*)(&temp_load_float[4]);
              *((float4*)(load_a_smem_addrs + 8)) = *(float4*)(&temp_load_float[8]);
              *((float4*)(load_a_smem_addrs + 12)) = *(float4*)(&temp_load_float[12]);
        }

        {
            qweight_reg[0] = qweight[load_b_gmem_addr];
            qweight_reg[1] = qweight[load_b_gmem_addr + 1];

            int zeros_scales_index = load_b_gmem_n * K / group_size + load_b_gmem_k / group_size;
            zeros_scales_reg[0] = zeros_scales[zeros_scales_index];
            // zeros_scales_reg[1] = zeros_scales[zeros_scales_index + 1];

            // dequant
            *(uint4*)(&weight_reg[0]) = dequantize_s4_to_fp16x2_v2(qweight_reg[0]);
            *(uint4*)(&weight_reg[8]) = dequantize_s4_to_fp16x2_v2(qweight_reg[1]);

            load_b_smem_addrs[0 * 16] = __half2float(__hmul(__hsub(weight_reg[0], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[1 * 16] = __half2float(__hmul(__hsub(weight_reg[1], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[2 * 16] = __half2float(__hmul(__hsub(weight_reg[2], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[3 * 16] = __half2float(__hmul(__hsub(weight_reg[3], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[4 * 16] = __half2float(__hmul(__hsub(weight_reg[4], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[5 * 16] = __half2float(__hmul(__hsub(weight_reg[5], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[6 * 16] = __half2float(__hmul(__hsub(weight_reg[6], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[7 * 16] = __half2float(__hmul(__hsub(weight_reg[7], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[8 * 16] = __half2float(__hmul(__hsub(weight_reg[8], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[9 * 16] = __half2float(__hmul(__hsub(weight_reg[9], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[10 * 16] = __half2float(__hmul(__hsub(weight_reg[10], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[11 * 16] = __half2float(__hmul(__hsub(weight_reg[11], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[12 * 16] = __half2float(__hmul(__hsub(weight_reg[12], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[13 * 16] = __half2float(__hmul(__hsub(weight_reg[13], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[14 * 16] = __half2float(__hmul(__hsub(weight_reg[14], zeros_scales_ptr[0]), zeros_scales_ptr[1]));
            load_b_smem_addrs[15 * 16] = __half2float(__hmul(__hsub(weight_reg[15], zeros_scales_ptr[0]), zeros_scales_ptr[1]));

            // load_b_smem_addrs[8 * 16] = __hmul(__hsub(weight_reg[8], zeros_scales_ptr[2]), zeros_scales_ptr[3]);
            // load_b_smem_addrs[9 * 16] = __hmul(__hsub(weight_reg[9], zeros_scales_ptr[2]), zeros_scales_ptr[3]);
            // load_b_smem_addrs[10 * 16] = __hmul(__hsub(weight_reg[10], zeros_scales_ptr[2]), zeros_scales_ptr[3]);
            // load_b_smem_addrs[11 * 16] = __hmul(__hsub(weight_reg[11], zeros_scales_ptr[2]), zeros_scales_ptr[3]);
            // load_b_smem_addrs[12 * 16] = __hmul(__hsub(weight_reg[12], zeros_scales_ptr[2]), zeros_scales_ptr[3]);
            // load_b_smem_addrs[13 * 16] = __hmul(__hsub(weight_reg[13], zeros_scales_ptr[2]), zeros_scales_ptr[3]);
            // load_b_smem_addrs[14 * 16] = __hmul(__hsub(weight_reg[14], zeros_scales_ptr[2]), zeros_scales_ptr[3]);
            // load_b_smem_addrs[15 * 16] = __hmul(__hsub(weight_reg[15], zeros_scales_ptr[2]), zeros_scales_ptr[3]);
       ...
       //      *(uint4*)(load_b_smem_addrs[1]) = *(uint4*)(&weight_reg[8]);
        ...
        nvcuda::wmma::load_matrix_sync(frag_a, &s_a[wid * 256], 16);
        nvcuda::wmma::load_matrix_sync(frag_b, &s_b[wid * 256], 16);
        nvcuda::wmma::mma_sync(frag_c, frag_a, frag_b, frag_c);

        nvcuda::wmma::load_matrix_sync(frag_a, &s_a[wid * 256 + 1024], 16);
        nvcuda::wmma::load_matrix_sync(frag_b, &s_b[wid * 256 + 1024], 16);
        nvcuda::wmma::mma_sync(frag_c, frag_a, frag_b, frag_c);

        nvcuda::wmma::load_matrix_sync(frag_a, &s_a[wid * 256 + 2048], 16);
        nvcuda::wmma::load_matrix_sync(frag_b, &s_b[wid * 256 + 2048], 16);
        nvcuda::wmma::mma_sync(frag_c, frag_a, frag_b, frag_c);

        nvcuda::wmma::load_matrix_sync(frag_a, &s_a[wid * 256 + 3096], 16);
        nvcuda::wmma::load_matrix_sync(frag_b, &s_b[wid * 256 + 3096], 16);
        nvcuda::wmma::mma_sync(frag_c, frag_a, frag_b, frag_c);
    ...
    nvcuda::wmma::store_matrix_sync(&smem[wid * BM * 16], frag_c, LDN, nvcuda::wmma::layout_t::mem_row_major);

    __syncthreads();

    int smem_c_m = tid / 16;
    int smem_c_n = tid % 16;
    int smem_c_addr = OFFSET(smem_c_m, smem_c_n, 16);
...
//     smem[smem_c_addr] = __hadd(smem[smem_c_addr], smem[smem_c_addr + BM * LDN * 3]);
    ...
    smem[smem_c_addr] = smem[smem_c_addr] + smem[smem_c_addr + BM * 16] 
                                        + smem[smem_c_addr + BM * 16 * 2];
                                        + smem[smem_c_addr + BM * 16 * 3];
