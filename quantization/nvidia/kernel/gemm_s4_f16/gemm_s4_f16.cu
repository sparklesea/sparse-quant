...
void GemmS4F16::Run(half* C,
    ...
    // use default cuda stream now
    impl_->Run(C, A, B, Q, m, n, k, group_size, type, algo_id, 0 /* st */, impl_->kernels_);
}

// qweight(A) * input(B) ---> output(C)
void GemmS4F16::RunWrapper(torch::Tensor input,
    ...
    uint* qweight_ptr = reinterpret_cast<uint*>(qweight.data_ptr<int>());
    half2* zeros_scales_ptr = reinterpret_cast<half2*>(zeros_scales.data_ptr<at::Half>());
    ...
    Run(output_ptr,
        ...
        input_features,
        group_size,
        turbomind::GemmS4F16::Type::kGemm,
        -1);
}
