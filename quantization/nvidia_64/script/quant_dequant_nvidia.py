...
def quant_nvidia(weight, input_features, output_features, group_size):
    qweight, zeros_scales = generate_quant_nvidia(weight, input_features, group_size)
    fp16_weight_dequant = dequant_to_fp16_nvidia(qweight, zeros_scales, group_size)

    reformat_weight = weight.reshape(-1, 4, 2).permute(0, 2, 1).reshape(output_features, input_features).clone()
    qweight, zeros_scales = generate_quant_nvidia(reformat_weight, input_features, group_size)

    return qweight, zeros_scales, fp16_weight_dequant

def quantize_tensor_nvidia(w, n_bit, q_group_size):
    org_w_shape = w.shape
    if q_group_size > 0:
...
def write_4bit_tensor_nvidia(w_q):
...
def deq_4bit_tensor_nvidia(w_int4):
...
def generate_quant_nvidia(weight_fp16, input_features, group_size):
    ...
    qweight_temp, scales_temp, zeros_temp = quantize_tensor_nvidia(weight_fp16[:, :end], 4, group_size)
    intweight.append(write_4bit_tensor_nvidia(qweight_temp))
...
def dequant_to_fp16_nvidia(qweight, zeros_scales, group_size):
    out_features = qweight.shape[0]
    in_features = qweight.shape[1] * 2

    fp16_weight = deq_4bit_tensor_nvidia(qweight).reshape(-1, group_size)
