...
def forward(q, k, v, sm_scale, lut, p_ref, BLOCK_M: int = 64, BLOCK_N: int = 64) -> torch.Tensor:
    dtype = q.dtype
...
q = torch.rand((1, 32, 6, 128))
q = q.to(torch.float16).to("cuda")
k = torch.rand((1, 32, 6, 128))
k = k.to(torch.float16).to("cuda")
v = torch.rand((1, 32, 6, 128))
v = v.to(torch.float16).to("cuda")
lut = torch.load("lut.pth")
lut = lut.to("cuda")
sm_scale = torch.load("sm_scale.pth")
qk_scale = sm_scale * 1.44269504
p = torch.zeros((64, 64))
p = p.to(torch.float16).to("cuda")


print(q.shape, k.shape, v.shape, lut.shape, sm_scale)


out = forward(q, k, v, sm_scale, lut, p)

k = k.transpose(2, 3)
p_ref = torch.matmul(q, k) * qk_scale
s = torch.softmax(p_ref, -1)
ref = torch.matmul(s, v)

print(p)
# p_ref = q[0][1].reshape((6, 128))
p_ref = p_ref[0][1].reshape((6, 6))
for i in range(6):
    # print(i, abs(p[i] - p_ref[i]).max().item())
    print(i, p[i])
    print(i, p_ref[i])
print("6", p[6])
