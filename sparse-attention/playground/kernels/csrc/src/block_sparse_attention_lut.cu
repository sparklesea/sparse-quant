...
std::vector<torch::Tensor> sparse_attention_prefill(
    ...
    sparse_attention_prefill_fwd_kernel<<<gridDim, blockDim>>>(Q.data_ptr<at::Half>(), K.data_ptr<at::Half>(), V.data_ptr<at::Half>(), sm_scale, out.data_ptr<at::Half>(), lut.data_ptr<int>(), m_i.data_ptr<float>(), l_i.data_ptr<float>(), bsz, head, seq_len, hidden_dim, lut_block, lut_size, NNZ, p.data_ptr<float>());
