...
"""
algorithmatically correct sparse attention
"""

def LlamaModel_static_attention_forward(
        self,
        ...
        for decoder_layer in self.layers:
            ...
            if seq_length == 1:
                ...
                # prefill
                attention_mask = decoder_layer.static_attention_mask[:, past_key_values_length:(seq_length+past_key_values_length), :(seq_length+past_key_values_length)][None, :, :, :].expand(batch_size, -1, -1, -1) * torch.finfo(inputs_embeds.dtype).min
...
def LlamaAttention_static_attention_forward(
        self,
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            ...
            if (attention_mask.size() != (bsz, 1, q_len, kv_seq_len)) and (attention_mask.size() != (bsz, self.num_heads, q_len, kv_seq_len)):
...
def LlamaModel_set_static_attention_mask(self, attention_mask_path: str):
    """
    ...
    for layer_index, layer in enumerate(self.layers):
        layer.register_buffer('static_attention_mask', ~predefined_attention_mask[layer_index])

def LlamaModel_use_static_attention(self):
    """
    Set the model instance to use head-wise static attention mask
    """
    self.set_static_attention_mask = MethodType(LlamaModel_set_static_attention_mask, self)
    self.forward = MethodType(LlamaModel_static_attention_forward, self)
    for layer in self.layers:
        layer.self_attn.forward = MethodType(LlamaAttention_static_attention_forward, layer.self_attn)

"""
efficient llama attention using block sparse
"""
from playground.kernels.block_sparse_attention_layout import create_block_sparse_attention_kernels

def LlamaAttention_block_sparse_forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
    ...
    attn_weights = self.sparse_dot_sdd(query_states, key_states) / math.sqrt(self.head_dim)
    # sparse softmax
    ...
    attn_output = self.sparse_dot_dsd(attn_weights, value_states)
    if output_attentions:
        batch_size = attn_weights.size(0)
        num_head = self.layout.size(0)
        attn_weights = block_sparse_to_dense(attn_weights, self.layout, batch_size, num_head, q_len, self.block_size)
    ### end block sparse implementation ###
...
def LlamaDecoderLayer_set_static_attention_layout(self, layout: Tensor, block_size: int, device: str = "cuda"):
    """
    Set the attention layout of the decoder layer

    layout: [num_head, num_block, num_block]
    block_size: int
    device: str
    """
    sparse_dot_sdd, sparse_dot_dsd, sparse_softmax = create_block_sparse_attention_kernels(layout, block_size, device)
    self.self_attn.layout = layout
    self.self_attn.block_size = block_size
    self.self_attn.sparse_dot_sdd = sparse_dot_sdd
    self.self_attn.sparse_dot_dsd = sparse_dot_dsd
    self.self_attn.sparse_softmax = sparse_softmax

def LlamaModel_use_block_sparse_attention(self):
    """
    Set the model instance to use block sparse attention instead of llama attention
    """
    # self.forward = MethodType(LlamaModel_pad_token_forward, self)

    # self.support_token_length = [self.config.max_position_embeddings]
    
    for layer in self.layers:
        layer.self_attn.forward = MethodType(LlamaAttention_block_sparse_forward, layer.self_attn)
        layer.set_static_attention_layout = MethodType(LlamaDecoderLayer_set_static_attention_layout, layer) # Deprecate soon. Add to support set_static_attention_layout in sparse_attention

"""
efficient llama attention using lut
"""
# from playground.kernels.block_sparse_attention_prefill_new import sparse_attention_prefill_new
...
def LlamaAttention_block_sparse_lut_forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
...
def LlamaDecoderLayer_set_static_attention_lut(self, lut, lut_for_head, block_size: int, device: str = "cuda"):
    """
    Set the attention layout of the decoder layer
...
def LlamaModel_use_block_sparse_attention_lut(self):
    """
    Set the model instance to use efficient attention instead of llama attention
    """
    
    for layer in self.layers:
        layer.self_attn.forward = MethodType(LlamaAttention_block_sparse_lut_forward, layer.self_attn)
...
def LlamaModel_get_attention_matrix_log(self):
    """
