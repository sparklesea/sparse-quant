...
class _sparse_attention_prefill_p_out(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, sm_scale, lut, LT) -> torch.Tensor:
        """
        input:
            q, k, v: (Z, H, N_CTX, L)
            sm_scale: float
        ...
        # print("entering _sparse_attention_prefill_p_out")
        ...
        if LT:
            ...
            P = block_sparse_ops.sparse_attention_prefill_p_64(q, k, sm_scale, lut)
...
class _sparse_attention_prefill(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, sm_scale, lut, LT: bool = True, BLOCK_M: int = 64, BLOCK_N: int = 64) -> torch.Tensor:
        """
        input:
            q, k, v: (Z, H, N_CTX, L)
            sm_scale: float
        ...
        _sparse_attention_prefill_fwd_kernel[grid](
            ...
            q.shape[0], q.shape[1], q.shape[2], LT, NNZ,
...
class _sparse_attention(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
            ctx, q, k, v, sm_scale, lut, LT: bool = True, BLOCK_M: int = 64, BLOCK_N: int = 64,
        ...
        # prefill
            ...
            return sparse_attention_prefill_p_out(q, k, v, sm_scale, lut, LT)
