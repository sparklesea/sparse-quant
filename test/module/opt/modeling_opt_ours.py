...
from module.modeling_utils import PreTrainedModel
...
from module.opt.configuration_opt import OPTConfig
from module.qlinear.qlinear import WALinear

# from playground.kernels.block_sparse_attention_layout import create_block_sparse_attention_kernels
# from playground.kernels.block_sparse_attention_lut import sparse_attention_prefill
# from playground.attention.sparse_attention import block_sparse_to_dense
...
class OPTAttention(nn.Module):
    ...
    def __init__(
        ...
        self.k_proj = WALinear(embed_dim, embed_dim, bias=bias, dtype=torch.int)
        self.v_proj = WALinear(embed_dim, embed_dim, bias=bias, dtype=torch.int)
        if not is_last:
            self.q_proj = WALinear(embed_dim, embed_dim, bias=bias, dtype=torch.int)
        else:
            self.q_proj = WALinear(embed_dim, embed_dim, bias=bias, w_bit=2, dtype=torch.int)
        self.out_proj = WALinear(embed_dim, embed_dim, bias=bias, dtype=torch.int)
        ...
        self.flash_attention_triton = None #sparse_attention_prefill

        self._is_flash_attn_triton_decode = False
        self.flash_attention_triton_decode = None #sparse_attention_decode
    ...
    def set_static_attention_layout(self, layout: torch.Tensor, block_size: int = 64):
    ...
    def set_flash_attention_triton(self, lut: torch.Tensor, block_size: int = 64):
        self._is_flash_attn_triton = True
    ...
    def forward(
        ...
        if self._is_sparse_implementation:
            ...
            if not hasattr(self, "sparse_dot_sdd"):
                self.sparse_dot_sdd, self.sparse_dot_dsd, self.sparse_softmax = create_block_sparse_attention_kernels(layout, self._block_size, hidden_states.device)
            # reshape 
        ...
        if ((not self._is_sparse_implementation) or self._debug) and (not self._is_flash_attn) and (not self._is_flash_attn_triton) and (not self._is_flash_attn_triton) and (not self._is_flash_attn_triton_decode):
        ...
        if self._is_sparse_implementation:
            ...
            lut = self.lut
            attn_output = self.flash_attention_triton(q, k ,v, self.scaling, lut, self.block_size, self.block_size).half()
        ...
        # print("self.out_proj: ", self.out_proj.weight, self.out_proj.weight.shape)

        return attn_output, attn_weights_reshaped, past_key_value


class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig, is_last: bool = False):
        super().__init__()
        ...
        self.fc1 = WALinear(self.embed_dim, config.ffn_dim, bias=config.enable_bias, dtype=torch.int)
        self.fc2 = WALinear(config.ffn_dim, self.embed_dim, bias=config.enable_bias, dtype=torch.int)
    ...
    def set_static_attention_layout(self, layout: torch.Tensor, block_size: int = 64):
        ...
        self.self_attn.set_static_attention_layout(layout, block_size)

    def set_flash_attention(self):
        self.self_attn.set_flash_attention()
    
    def set_static_attention_lut(self, lut: torch.Tensor, lut_for_head: torch.Tensor, block_size: int = 64):
        self.self_attn.set_flash_attention_triton(lut, block_size)
    ...
    def forward(
        ...
        # TODO 
        self._is_decode = _is_decode
        
        # Replace the attention mask if the rule is set
        if self._set_static_attention_mask and (not _is_decode):
        ...
        hidden_states = hidden_states.unsqueeze(0) #TODO 
...
class OPTPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        # if isinstance(module, nn.Linear) or isinstance(module, WALinear):
...
class OPTDecoder(OPTPreTrainedModel):
    ...
    def __init__(self, config: OPTConfig):
        ...
        # self.layers = nn.ModuleList([OPTDecoderLayer(config, False) for _ in range(config.num_hidden_layers)])
        self.layers = nn.ModuleList([OPTDecoderLayer(config, False) for _ in range(config.num_hidden_layers - 1)])
        self.layers.append(OPTDecoderLayer(config, True))
        # self.last_layer = OPTDecoderLayer(config, True)
...
class OPTForCausalLM(OPTPreTrainedModel):
    ...
    def __init__(self, config: OPTConfig):
        ...
        # self.lm_head = WALinear(config.word_embed_proj_dim, config.vocab_size, bias=False, dtype=torch.uint8)
...
class OPTForSequenceClassification(OPTPreTrainedModel):
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.model = OPTModel(config)
        self.score = nn.Linear(config.word_embed_proj_dim // 2, self.num_labels, bias=False)
...
class OPTForQuestionAnswering(OPTPreTrainedModel):
    ...
    def __init__(self, config: OPTConfig):
        super().__init__(config)
        self.model = OPTModel(config)
        self.qa_outputs = nn.Linear(config.word_embed_proj_dim // 2, 2)
...
def OPTModel_grad_analysis_forward(
        self,
        hidden_states: torch.Tensor,
        key_value_states: Optional[torch.Tensor] = None,
        past_key_value: Optional[Tuple[torch.Tensor]] = None,
...
def OPTModel_get_attention_matrix_log(self):
    """
    ...
    for layer in self.decoder.layers:
        grad_A.append(layer.self_attn.attn_weights.grad.detach().to('cpu'))
        mat_A.append(layer.self_attn.attn_weights.detach().to('cpu'))
    # -> (bs,32,32,seq_len,seq_len)
    if len(grad_A[0].shape) > 3: # batch_size == 4
        grad_A = torch.stack(grad_A, dim=1)
        mat_A = torch.stack(mat_A, dim=1)
    # -> (1,32,32,seq_len,seq_len)
    else:   # batch_size == 1
        grad_A = torch.stack(grad_A, dim=0).unsqueeze(0)
        mat_A = torch.stack(mat_A, dim=0).unsqueeze(0)
    ...
    effects = mat_A*(grad_A-torch.sum(multiply, dim=-1, keepdim=True))/(delta+1-mat_A)
    # deal with the first row of effects
    effects[..., 0, :] = multiply[..., 0, :]

    return {'grad': grad_A.cpu(), 'matrix': mat_A.cpu(), 'sum_effect': effects.cpu()}

def OPTModel_use_attention_matrix_grad_log(self):
    """
    Set the model instance to use flash attention instead of llama attention
    """
    self.attention_matrix_log = defaultdict(list)
    for layer in self.decoder.layers:
        layer.self_attn.forward = MethodType(OPTModel_grad_analysis_forward, layer.self_attn)
    self.get_attention_matrix_log = MethodType(OPTModel_get_attention_matrix_log, self)